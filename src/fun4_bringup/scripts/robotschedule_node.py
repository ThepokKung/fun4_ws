...
class RobotscheduleNode(Node):
    ...
    def Targetmanual_callback(self, request, response):
        ...
        if self.controll_state != 2:
            if self.mode_sate == 1:
                # Prepare the target for the IK service call
                ...
                # self.want_ink_clinet.wait_for_service(1)

                # """response from future_ink_result is true or false"""
    ...
    def ink_result_response(self, future, response):
        ...
        # self.get_logger().info(f"Sending response: success={response.success}, message={response.message}")
        # return response
